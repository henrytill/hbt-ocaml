...
**`absorb` passes only `other.created_at` into `update` and discards `other.updated_at`.** This survived #49: making the field a `Time_set` fixed the duplication, but `update` still takes a single `Time.t`, so there is nowhere for the incoming history to go. It is reachable from one HTML file - `Html.entity_of_attrs` sets `updated_at` from `LAST_MODIFIED`, so two mentions of a URI with the same `ADD_DATE` and different `LAST_MODIFIED` keep whichever came first - and it shows in the `LAST_MODIFIED` this implementation emits, not only in the YAML. It is #56, with henrytill/hbt-rs#64 the companion; hbt-go and hbt-hs already union. No fixture pins it.

**`update` takes one timestamp and records it, and that is the whole of the history it can build.** The two `List.sort Time.compare` calls it used to carry are gone with #49 - a `Time_set` is ordered by construction - so the function is now just a three-way compare against `created_at`. The `c = 0` branch is the #57 rule and is deliberately not an optimization; see the first bullet above.
