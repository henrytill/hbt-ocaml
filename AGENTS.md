...
- **An entity's `updated_at` never contains its `created_at`, and this is maintained at parsing and decoding, not only by the merge.** `normalize` is the whole of the normal form, settled for all four implementations as henrytill/hbt-data#38. Three places end in it - the three that take a history from input: `absorb`'s merging branch, `t_of_yaml`, and `Html.entity_of_attrs` (HTML reads `ADD_DATE` and `LAST_MODIFIED` independently, so one anchor may state the same instant in both - the `html/bookmarks_simple` shape). **Merging is field-wise, then normalized**, so do not give `merged_timestamps` a removal of its own; two spellings of one rule is what a later change would have to keep in step.
- **`make` deliberately does not normalize**, even though its `?updated_at` could carry a repeat. This is the one hole in the invariant and it is on purpose. No production caller passes `?updated_at` - `of_post` and the Markdown parser call `make` without it, and the HTML parser and the decoder fold the record directly - so nothing the program builds is non-normal. What the hole buys is that `core/collection_test.ml`, which is outside the module and so has no other way in, can still construct the un-normalized values the merge properties must range over. Associativity and the identical-entity guard are claims about every value of the type, not only the reachable ones; normalizing in `make` leaves both tests passing while testing nothing. This is the same trap that narrowed a QuickCheck generator in hbt-hs#55. It is also the reason this implementation does not close the representation the way henrytill/hbt-hs#54 and henrytill/hbt-rs#85 propose for theirs.
- **A creation time can be absent, and an absent one contributes nothing.** `created_at` is a `Time.t option`, so an anchor with no `ADD_DATE` parses to an entity with no creation time rather than one created at the epoch, and `merged_timestamps` makes absence the identity: a dated mention merged with an undated one keeps its own instant, and neither operand contributes an absence to the history. Two undated mentions stay undated. This is henrytill/hbt-data#37, pinned by `html/bookmarks_undated`, `html/bookmarks_undated_merged` and `html/bookmarks_undated_both`.
- **Absence has a wire representation**, which is what makes the rule survive a round-trip: `createdAt` is omitted when absent, as `lastVisitedAt` is, and the schema no longer requires it. Before that, an undated entity serialized as `createdAt: 0` and decoded back as one created on 1970-01-01, so the same collection merged differently depending on whether it had passed through YAML -- and this implementation is the one where that is a real CLI path, since it accepts `-f yaml`. A creation time of `0` is a real instant: it still serializes and still exports `ADD_DATE="0"`, because the formatter keys on the option rather than on the rendered string.
- **The merged timestamps are every history and every creation time in the merge, minus the smallest creation time**, which becomes `created_at`. `merged_timestamps` puts both creation times in and `normalize` takes the winner out. The rule is the one that keeps merging associative, which is how henrytill/hbt-data#36 settled it for all four implementations: putting both creation times back into the history before removing the winner is what makes any bracketing of a sequence of mentions agree. Removing the winner only when the two creation times differ is *not* associative, and neither is removing every update at or below `created_at`; that issue carries both counterexamples.
- **A timestamp equal to `created_at` is not recorded** - an "update" that merely repeats the creation instant carries no information. Settled as henrytill/hbt-go#57, and the rule above extends it to the instant that *becomes* `created_at`, including one a merge did not move (`html/bookmarks_merged_repeat`).
...
- **Identical entities short-circuit.** `absorb` returns `existing` unchanged when `equal other existing`. This is still **not** an optimization - the rule drops an update equal to `created_at`, so without the guard the same mention twice would not read like it once - but what it protects has moved. Since the parse path normalizes, `html/bookmarks_simple` no longer produces that shape, so the only way to reach it is `make` with an `?updated_at` carrying the repeat. That is exactly what `absorb an identical entity` in `core/collection_test.ml` builds, and why `make` does not normalize. No fixture covers it, because no fixture duplicates an anchor carrying `LAST_MODIFIED`. hbt-go and hbt-rs guard the same way, as does hbt-hs in its own `absorb`.
- **Multi-valued fields are sets.** `names`, `labels`, `extended`, and `updated_at` are `Name_set`, `Label_set`, `Extended_set`, and `Time_set`. This is the load-bearing decision: the equality guard covers only identical entities, so two entities that differ in *any* field bypass it, and anything appended would land once per occurrence - making the output depend on how many times the input mentioned a bookmark. `extended` became a set in #48, `updated_at` in #49.
- **Ordering is a consequence, not a step.** `Set.Make` keeps elements sorted by construction, so the single representative the HTML formatter needs is `min_elt_opt` - or `max_elt_opt`, where `LAST_MODIFIED` wants the latest update - not a sort-then-take-head.

**Both histories are unioned.** `absorb` used to pass only `other.created_at` into `update` and discard `other.updated_at`, so two mentions of a URI with the same `ADD_DATE` and different `LAST_MODIFIED` kept whichever came first. That was #56, with henrytill/hbt-rs#64 the companion; both are fixed, and `html/bookmarks_incoming_update` pins it.
...
- **A module per field, not a bare primitive.** `Name`, `Label`, and `Extended` are all `type t = string` over the same small signature (`entity.mli` has it), and are deliberately *not* factored together: they exist so the type checker catches field mix-ups, which sharing an implementation would undo. The `Set.Make` + `pp`/`t_of_yaml`/`yaml_of_t` boilerplate around them is a known repetition - four copies since `Time_set` landed with #49 - tracked in #52 and deliberately not acted on; leave it alone unless that issue is the task.
- **`Name`, `Label` and `Extended` are nonempty.** `of_string` returns `t option` and `of_string_exn` raises `Entity.Empty`; a producer holding untrusted text uses the former, the deserialization path the latter. That naming is the house convention - a raising function is marked `_exn` (`Yaml_ext.map_array_exn`, `int_of_float_exn`) and an option-returning one carries no suffix (`Collection.id`, `Data.detect_input_format`) - and it is the stdlib's spelling reversed, so don't reach for `_opt`. This is why `Collection.update_labels` cannot use `Label_set.map` - a total substitution has nowhere to put a label whose mapped value is empty, which is a deletion (#50). The set decoders drop empty entries rather than raising, matching hbt-go's reader; the scalar `t_of_yaml` raises.
...
3. Decide the merge rule and implement it in `absorb`. If it is multi-valued, make it a set - see [Merge Semantics](#merge-semantics).
