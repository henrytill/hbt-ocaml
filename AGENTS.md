...
- **Identical entities short-circuit.** `absorb` returns `existing` unchanged when `equal other existing`. This is **not** an optimization: the rule above would drop an update equal to `created_at`, so without the guard an entity parsed from `LAST_MODIFIED == ADD_DATE` - the `html/bookmarks_simple` shape - would lose that update on meeting a byte-identical duplicate. No fixture covers it, because no fixture duplicates an anchor carrying `LAST_MODIFIED`; `absorb an identical entity` in `core/collection_test.ml` is what pins it, as it does in hbt-go and hbt-rs.
