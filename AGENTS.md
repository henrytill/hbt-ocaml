...
- **An entity's `updated_at` never contains its `created_at`, and this is maintained at parsing and decoding, not only by the merge.** `normalize` is the whole of the normal form, settled for all four implementations as henrytill/hbt-data#38. Three places end in it - the three that take a history from input: `absorb`, `t_of_yaml`, and `Html.entity_of_attrs` (HTML reads `ADD_DATE` and `LAST_MODIFIED` independently, so one anchor may state the same instant in both - the `html/bookmarks_simple` shape). **Merging is field-wise, then normalized**, so do not give `merged_timestamps` a removal of its own; two spellings of one rule is what a later change would have to keep in step.
