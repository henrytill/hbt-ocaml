...
**`absorb` passes only `other.created_at` into `update` and discards `other.updated_at`.** Merging two collections that each carry update history keeps only the existing side's. This survived #49: making the field a `Time_set` fixed the duplication, but `update` still takes a single `Time.t`, so there is nowhere for the incoming history to go. Unioning it is a one-line change and a cross-language decision that no fixture pins; it is not filed anywhere yet.
